...
public class CreateOrderKafkaPublisher implements OrderCreatedPublisher {
    ...
    public void publish(OrderCreatedEvent domainEvent) {
        ...
        try {
            paymentRequestAvroModel = orderMessagingDataMapper.orderCreatedEventToPaymentRequestAvroModel(domainEvent);

            kafkaProducer.send(orderServiceConfig.getPaymentRequestTopicName(),
                    orderId,
                    paymentRequestAvroModel,
                    kafkaMessageCallbackHelper
                            .getKafkaCallback(orderServiceConfig.getPaymentResponseTopicName(),
                                    paymentRequestAvroModel,
                                    orderId
                            )
            );
